...
def accuracy(entered: str, source: str) -> float:
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    entered_length = len(entered_words)
    ...
    if entered_length == 0:
        ...
        return 0.0
    correct_count = 0
    for entered_word, source_word in zip(entered_words, source_words):
        if entered_word == source_word:
            correct_count += 1
    return correct_count / entered_length * 100
    # END PROBLEM 3
...
def wpm(entered: str, elapsed: int) -> float:
    ...
    if entered_length == 0:
        return 0.0
    return entered_length / 5 * 60 / elapsed
    # END PROBLEM 4
...
def autocorrect(
    ...
    for word in word_list:
        temp_diff = diff_function(entered_word, word, limit)
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    def judge(entered_slice, source_slice, character_diff):
        if character_diff + len_diff > limit:
            return character_diff
        if not entered_slice or not source_slice:
            return character_diff
        if entered_slice[0] == source_slice[0]:
            return judge(entered_slice[1:], source_slice[1:], character_diff)
        else:
            return judge(entered_slice[1:], source_slice[1:], character_diff + 1)

    return judge(entered, source, 0) + len_diff
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for word_idx in w_idxs:
        fastest_player = 0
        fastest_time = get_time(times, 0, word_idx)
        for player_idx in pl_idxs:
            temp_time = get_time(times, player_idx, word_idx)
            if temp_time < fastest_time:
                fastest_time = temp_time
                fastest_player = player_idx
        fastest[fastest_player].append(words[word_idx])
    return fastest
    # END PROBLEM 10
