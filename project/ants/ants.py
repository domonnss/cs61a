...
from jedi.plugins.stdlib import _implemented
from pexpect.FSM import DoOperator
...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.is_double is False and self is not None:
            self.damage *= 2
        self.is_double = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        current: Place | None = self.place
        dis_count = 0
        while current is not None and not current.is_hive:
            if dis_count >= self.lower_bound and dis_count <= float(self.upper_bound):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate: GameState):
        if self.cooldown != 0:
            ...
            current_place = self.place
            if current_place is not None:
                target_bee = random_bee(current_place.bees)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        while current_place is not None:
            ...
            next_ant: Ant | None = None
            if next_place is not None:
                next_ant = next_place.ant
            if next_ant is not None:
                next_ant.double()
                if (
                    isinstance(next_ant, ContainerAnt)
                    and next_ant.ant_contained is not None
                ):
