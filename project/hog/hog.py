...
def boar_brawl(player_score, opponent_score):
    ...
    _, player_ones_place = get_tens_and_ones(player_score)
    opponent_tens_place, _ = get_tens_and_ones(opponent_score)
    diff = abs(player_ones_place - opponent_tens_place)
    return max(3 * diff, 1)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    same_flag = True
    roll_nums = strategy(0, 0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != roll_nums:
                same_flag = False
    return same_flag
    # END PROBLEM 7
...
def make_averaged(original_function, times_called=1000):
    ...
    # TERM: *args, 未定参数个数
    # NOTE: 嵌套,higer order function,使用封装技术,方便调用
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    # NOTE: 想封装哪个就写哪个函数,例如这里封装了roll_dice
    averaged_function = make_averaged(roll_dice, times_called)
    max_average = 0
    max_num = 0
    for i in range(1, 11):
